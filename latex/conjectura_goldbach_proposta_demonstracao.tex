...
\begin{document}
	...
	\begin{proof}
		...
		Logo, necessariamente, existe pelo menos um primo \(p\) tal que \(p\) não divide \(m\). Consequentemente, como as sequências \(a_n = p + S_n(p)\) são construídas excluindo os ímpares compostos em \(S_n(p)\) e, por sua vez, também os múltiplos de \(p\), o número \(m\) pode pertencer a algum subconjunto \(A_p = \{p + q \mid q \in Q_p\}\), onde \(q\) percorre os primos da sequência \(S_n(p)\).
