...
\begin{document}
	...
	\begin{definition}[Conjunto \(Q_p\)]
		...
		onde \(S_n(p)\) é a sequência dos números ímpares construídos com base em simetrias desses ímpares com cada número par maior que dois, conforme definido anteriormente. O conjunto \(Q_p\) representa, portanto, todos os primos que pertencem à sequência \(S_n(p)\) associada a cada primo \(p\).
	\end{definition}
	...
	\begin{proposition}[Simetria de Propagação a partir de um Par Menor]\label{prop:propagacao_a_partir_par_menor}
		...
		e portanto \(m \in A_p\), desde que \(q \pm 2k\) seja primo.
	\end{proposition}
	...
	\begin{proof}
		...
		Se \(q + 2k\) for primo e \(q + 2k > p\), então, como \(S_n(p)\) é a sequência dos ímpares maiores que \(p\), segue que \(q + 2k \in S_n(p)\) e, sendo primo, \(q + 2k \in Q_p\).
		
		Logo, \(m = p + (q + 2k) \in A_p\).
		
		O mesmo argumento vale para \(m < a\), com \(m = a - 2k = p + (q - 2k)\), assumindo \(q - 2k\) primo, maior que \(p\).
	...
	\begin{remark}
		 Isso ocorre porque a diferença \(m - a = 2k\) pode ser compensada por um deslocamento simétrico no elemento \(q \in Q_p\), preservando a estrutura de \(A_p\) e reforçando sua propagação.
	\end{remark}
	...
	\begin{proof}
		...
		Contudo, isso é um absurdo, pois já foi demonstrado nas Proposições \ref{prop: produto_p_menores_a_meio} e \ref{prop:existencia_de_primos_nao_divisores} que não há como \(m\) ser múltiplo de todos os primos \( p_i \) tais que \( p_i < m/2 \). 
		
		Logo, necessariamente, existe pelo menos um primo \(p\) tal que \(p\) não divide \(m\). Como \(m\) pode ser escrito da forma \(p + S_n(p)\) e \(p \nmid m \), então \(p \nmid p +  S_n(p)\). Pelo Lema \ref{lema:multip}, \(p \nmid p +  q\) sempre que \(q\) é primo ou \(q\) é um ímpar composto não múltiplo de \(p\). 
		
		Portanto, se \(p \nmid m \), então \(q\) pode ser primo, reforçando que \(m\) deve pertencer a algum subconjunto \(A_p = \{p + q \mid q \in Q_p\}\), onde \(q\) percorre os primos da sequência \(S_n(p)\).
