...
\begin{document}
	...
	\begin{abstract}
		...
		A Se\c{c}\~ao~12 apresenta verifica\c{c}\~ao computacional preliminar do peso geom\'etrico
		$\Omega_N(n)$, mostrando que as distribui\c{c}\~oes de $\Omega_N$ sobre primos ($\mathbb{P}_1$)
		e semiprimos ($\mathbb{P}_2$) s\~ao estatisticamente distingu\'iveis para $N$ livre de
		obstru\c{c}\~oes modulares --- evid\^encia emp\'irica de consist\^encia com a hip\'otese do
		crivo geom\'etrico.
	\end{abstract}
	\begin{quote}
		\textbf{Nota do Autor.} 
		...
		O objetivo do texto não é reivindicar uma demonstração da Conjectura de Goldbach, mas desenvolver e analisar frameworks heurísticos, correspondências estruturais e possíveis direções de pesquisa relacionadas às partições aditivas por números primos.
	...
	A Se\c{c}\~ao~5 estabelece o elo entre trios primos e a cobertura de $\bigcup_p A_p$.
	A Se\c{c}\~ao~6 analisa a densidade de cobertura unificada.
	A Se\c{c}\~ao~7 discute a lacuna central e perspectivas.
	A Se\c{c}\~ao~8 apresenta a redu\c{c}\~ao dimensional da grade como dobramento da fita.
	A Se\c{c}\~ao~9 desenvolve a an\'alise por m\'etodos de crivo e o problema da paridade.
	A Se\c{c}\~ao~10 formaliza a grade como sistema erg\'odico e enuncia as conjecturas principais.
	A Se\c{c}\~ao~11 delineia o programa de pesquisa do crivo geom\'etrico.
	A Se\c{c}\~ao~12 apresenta verifica\c{c}\~ao computacional do peso geom\'etrico $\Omega_N(n)$.
	A Se\c{c}\~ao~13 conclui.
	...
	\begin{remark}[Consist\^encia com a nota\c{c}\~ao de~\cite{bandeira2026estrutura}]
		Em~\cite{bandeira2026estrutura}, $S_n(p)$ \'e apresentada por dois sub-casos:
		$p+4n$ e $p+4n+2$ (para $n\in\NN$). A Defini\c{c}\~ao~2.4 acima \'e
		equivalente, reunindo ambos em uma uni\~ao expl\'icita.
		Todo \'impar $q > p$ escreve-se como $q = p + 2m$ para algum $m \geq 1$:
		se $m$ \'e par ($m = 2n$), ent\~ao $q \in \{p+4n\}$;
		se \'impar ($m = 2n+1$), ent\~ao $q \in \{p+4n+2\}$.
		Portanto $S_n(p)$ cobre exatamente todos os \'impares maiores que $p$
		em ambas as nota\c{c}\~oes.
	\end{remark}
	...
	\begin{proof}
		...
		\]
		A integral do lado direito diverge. De fato, pela substitui\c{c}\~ao
		$u = \ln x$ (i.e.\ $x = e^u$, $dx = e^u\,du$):
		\[
		\int_{3}^{\infty}\frac{dx}{(\ln x)^3}
		= \int_{\ln 3}^{\infty}\frac{e^u}{u^3}\,du.
		\]
		Como $e^u/u^3 \to +\infty$ quando $u\to\infty$, existe $u_0\geq\ln 3$ tal que
		$e^u/u^3 \geq 1$ para todo $u\geq u_0$. Portanto
		\[
		\int_{\ln 3}^{\infty}\frac{e^u}{u^3}\,du
		\;\geq\; \int_{u_0}^{\infty} 1\,du = +\infty.
		\]
		Observa\c{c}\~ao: a compara\c{c}\~ao direta $1/(\ln x)^3 > 1/x$ \emph{n\~ao} \'e
		v\'alida para todo $x\geq 3$ (falha na regi\~ao $x\approx 10$--$100$,
		onde $(\ln x)^3 > x$), e por isso n\~ao \'e utilizada aqui.
		\qedhere
	\end{proof}
	
	\begin{proposition}[Saturação estrutural via Borel--Cantelli --- \emph{condicional}]
		\label{prop:saturacao}
		\emph{Assumindo} a Hip\'otese de Cram\'er (Hip\'otese~4.1), a probabilidade de que
		$E_i$ ocorra infinitas vezes \'e~$1$:
		\[
		\bP\!\bigl(\limsup_{i\to\infty} E_i\bigr) = 1.
		\]
		Este resultado \'e condicional: \'e um teorema relativo ao modelo probabil\'istico
		de Cram\'er, n\~ao um resultado incondicional sobre os n\'umeros primos.
	\end{proposition}
	\begin{proof}
		Pelo Lema~\ref{lem:diverge}, $\sum\bP(E_i) = \infty$.
		Pela Hip\'otese~4.1, janelas com $|i-j|\geq 3$ s\~ao assintoticamente independentes.
		O Segundo Lema de Borel--Cantelli~\cite{durrett2019} --- que exige, al\'em da
		diverg\^encia da s\'erie, alguma forma de independ\^encia ou quasi-independ\^encia
		--- aplica-se e garante recorr\^encia quase certa.
		Janelas sobrepostas ($|i-j|<3$) s\~ao tratadas pelo fator de corre\c{c}\~ao $1/3$
		e n\~ao afetam a diverg\^encia nem a conclus\~ao de Borel--Cantelli.
	\end{proof}
	...
	\begin{remark}[Caminhos al\'em da paridade]
		...
		\end{itemize}
		O framework deste trabalho sugere uma quarta dire\c{c}\~ao: usar os pesos geom\'etricos
	...
	\section{Formalização Dinâmica: A Grade como Sistema Ergódico}
	\label{sec:ergodica}
	...
	Esta abordagem traduz o problema da existência de configurações primas (como a diagonal totalmente prima $D_1$) em um problema de \textit{recorrência múltipla} em um espaço de medida.
	
	\subsection{O Espaço Simbólico e o Operador de Shift}
	...
	\begin{equation}
		x(n) = \begin{cases} 
			1 & \text{se } 2n-1 \in \mathcal{P} \\
			0 & \text{caso contrário}
		\end{cases}
	\end{equation}
	
	Definimos o operador de \textit{shift} contínuo $T: X \to X$ como $(Tx)(n) = x(n+1)$. Este operador modela o deslocamento ao longo da sequência de números ímpares (um passo de $+2$ nos inteiros).
	...
	Portanto, a exist\^encia de um sucesso em $D_1$ na janela $W_i$ equivale a:
	\begin{equation}
		x(i) \cdot x(i+\Delta) \cdot x(i+2\Delta) = 1,
	\end{equation}
	ou seja, a encontrar tr\^es termos da sequ\^encia indicadora, espa\c{c}ados de
	$\Delta = N+1$, todos iguais a~1.
	Em nota\c{c}\~ao de shift: $(T^k x)(n) = x(n+k)$, de modo que
	$x(i+k) = (T^k x)(i)$; assim a condi\c{c}\~ao acima escreve-se
	equivalentemente $x(i)\cdot(T^\Delta x)(i)\cdot(T^{2\Delta}x)(i)=1$.
	
	\subsection{Medida Empírica e o Filtro de Módulos (W-Trick)}
	...
	Seja $\mu_N$ a medida de probabilidade emp\'irica suportada na malha $G_{3,N}$.
	Para evitar obstru\c{c}\~oes triviais de congru\^encia --- por exemplo,
	se $3 \mid \Delta$ os tr\^es \'indices $i, i+\Delta, i+2\Delta$ caem na mesma classe
	residual m\'od~$3$, tornando imp\'ossivel a primalidade simult\^anea para primos $>3$
	--- aplicamos o \emph{$W$-trick}: fixamos $W = \prod_{p \leq w} p$ para
	$w = w(N) \to \infty$ lentamente (e.g.\ $w = \log\log N$), e restringimos
	a medida $\mu_N$ aos \'indices $i$ para os quais $i, i+\Delta, i+2\Delta$
	pertencem a classes residuais $b_1, b_2, b_3 \pmod{W}$ com $\gcd(b_j, W)=1$.
	Sob esta restri\c{c}\~ao, os primos comportam-se de forma pseudo-aleat\'oria
	com densidade relativa $\phi(W)/W > 0$ no subconjunto filtrado
	(onde $\phi$ \'e a fun\c{c}\~ao de Euler).
	
	\subsection{Conjecturas Ergódicas para a Estrutura Goldbach}
	
	A tradu\c{c}\~ao da grade $G_{3,N}$ para o sistema din\^amico $(X, \mu_N, T)$ permite
	formular as conjecturas centrais do programa erg\'odico, substituindo a diverg\^encia
	de Borel--Cantelli por converg\^encia em m\'edia erg\'odica sobre translações.
	
	\begin{hypothesis}[Recorr\^encia Estrutural na Malha]
		\label{hyp:ergodica}
		Seja $A = \{x\in X : x(0)=1\}$ o conjunto cil\'indrico de sequ\^encias com primo
		na posi\c{c}\~ao inicial, e $\Delta = N+1$.
		Para todo $N$ suficientemente grande (livre de obstru\c{c}\~oes modulares locais
		ap\'os o $W$-trick):
		\[
		\liminf_{M \to \infty} \frac{1}{M} \sum_{i=1}^{M}
		\mu_N\!\bigl(T^{-i}A \cap T^{-(i+\Delta)}A \cap T^{-(i+2\Delta)}A\bigr)
		\;\geq\; \mathfrak{S}(D_1, N) \cdot \left(\frac{c}{\ln N}\right)^3 > 0,
		\]
		onde $c>0$ \'e uma constante absoluta e $\mathfrak{S}(D_1,N)$ \'e a constante de
		correla\c{c}\~ao estrutural da S\'erie Singular de Hardy--Littlewood
		associada \`a progress\~ao com passo $\Delta$.
	\end{hypothesis}
	
	\begin{remark}[Por que a m\'edia sobre $i$ \'e necess\'aria]
		O somatório $\frac{1}{M}\sum_{i=1}^M$ \'e a m\'edia de Ces\`aro sobre
		\emph{transla\c{c}\~oes} do sistema: o argumento de $\mu_N$ muda com $i$,
		pois $T^{-i}A$ \'e o conjunto de sequ\^encias com primo na posi\c{c}\~ao $i$.
		Sem essa m\'edia, a express\~ao seria uma medida constante repetida $M$ vezes,
		colapsando trivialmente. A forma correta \'e exatamente a m\'edia erg\'odica de
		m\'ultipla recorr\^encia (cf.\ Teorema de Furstenberg para averages de Ces\`aro).
	\end{remark}
	
	\begin{hypothesis}[Densidade de Cobertura via Ergodicidade]
		\label{hyp:cobertura-ergodica}
		Se a Hip\'otese~\ref{hyp:ergodica} for verdadeira, ent\~ao a proje\c{c}\~ao
		das recorr\^encias de volta aos inteiros --- via as Proposi\c{c}\~oes~\ref{prop:aditiva}
		e~\ref{prop:elo} --- garante que a densidade superior do conjunto de pares n\~ao cobertos
		tende a zero:
		\[
		\overline{d}\!\Bigl(\calA \setminus \bigcup_{p \leq P} A_p\Bigr) \;\to\; 0
		\quad\text{quando } P\to\infty.
		\]
	\end{hypothesis}
	
	\begin{remark}[Rela\c{c}\~ao com resultados conhecidos e com a lacuna]
		A Hip\'otese~\ref{hyp:cobertura-ergodica} \'e deliberadamente mais fraca que Goldbach:
		``quase todo par \'e soma de dois primos'' j\'a \'e conhecido por outros meios
		(m\'etodo do c\'irculo; o conjunto excepcional tem tamanho $o(N)$).
		O interesse est\'a em \emph{como} chegar a esse resultado via geometria da grade,
		sem crivos multiplicativos, e em se o m\'etodo pode ser refinado para eliminar
		inteiramente o conjunto excepcional.
		A lacuna entre ``densidade nula'' (Hip\'otese~\ref{hyp:cobertura-ergodica}) e
		``conjunto vazio'' (Goldbach) \'e o passo que permanece aberto.
	\end{remark}
	
	Esta formaliza\c{c}\~ao exp\~oe o caminho para provar a validade da malha $G_{3,N}$
	sem recorrer a fun\c{c}\~oes multiplicativas, utilizando a invari\^ancia de
	transla\c{c}\~ao e a estabilidade din\^amica das constela\c{c}\~oes de \'impares na reta.
	
	\section{Direções Futuras: O Paradigma do Crivo Geométrico}
	\label{sec:crivo_geometrico}
	
	O que segue é um programa especulativo, não um resultado estabelecido. A definição de 
	$\Omega_N(n)$
	é conceitual e não constitutiva de uma prova. O objetivo é sugerir um caminho para futuras investigações que possa contornar o problema da paridade, mas não há, neste momento, demonstração de que tal peso seja computável ou eficaz.
	...
	\medskip
	\noindent\textit{Nota de revisão (maio de 2026):} o nó computacional
	identificado nesta seção foi resolvido em~\cite{bandeira2026ancora}, onde o
	Invariante Âncora de~\cite{bandeira2026propriedade} --- a propriedade
	$f(2,i{+}1)=S(i)/3$ --- é usado para construir um estimador oracle-free
	$\Omega^*_N(n)$, livre de qualquer teste de primalidade sobre vizinhos.
	Verificação computacional em $G_{3,N}$ para $N$ de $10^3$ a $10^5$ mostra
	que $\Omega^*_N$ discrimina $P_1$ de $P_2$ em todos os $N$ testados, incluindo
	os $N$ com obstruções modulares onde $\Omega_N$ falha --- confirmando que o
	estimador oracle-free captura um sinal aritmético independente.
	
	%% ===========================================================
	\section{Verificação Computacional do Peso Geométrico $\Omega_N(n)$}
	\label{sec:computacional}
	
	Esta seção apresenta evidência computacional preliminar sobre a capacidade
	do peso geométrico $\Omega_N(n)$ (Definição~11.2) de distinguir números
	primos ($\mathbb{P}_1$) de semiprimos ($\mathbb{P}_2 = \{pq : p,q \in \mathbb{P}\}$).
	A questão é relevante para a viabilidade do crivo geométrico proposto na
	Seção~11: se $\Omega_N$ for estatisticamente cego à paridade, o programa
	colapsa; se as distribuições de $\Omega_N$ sobre $\mathbb{P}_1$ e $\mathbb{P}_2$
	forem distinguíveis, a hipótese mantém-se aberta.
	
	\subsection{Metodologia}
	
	Implementamos a grade $G_{3,N}$ computacionalmente para $N$ variando entre
	$10^3$ e $10^5$. Para cada célula $n \in G_{3,N}$, calculamos $\Omega_N(n)$
	pela fórmula~(6), classificamos $n$ como $\mathbb{P}_1$ (primo), $\mathbb{P}_2$
	(produto de exatamente dois primos com multiplicidade) ou composto
	($\Omega(n) \geq 3$) via crivo de Eratóstenes, e aplicamos dois testes não
	paramétricos para comparar as distribuições de $\Omega_N$ entre as classes:
	o teste de Kolmogorov--Smirnov (KS), sensível a diferenças globais de
	distribuição, e o teste de Mann--Whitney (MW), sensível a diferenças de
	posição/mediana.
	
	\subsection{Resultados}
	
	A Tabela~\ref{tab:omega} resume os resultados para uma seleção representativa
	de~$N$.
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{r l c c c}
			...
			\midrule
			1800   & $2^3\cdot3^2\cdot5^2$               & 0,118        & 0,302        & Não    \\
			2400   & $2^5\cdot3\cdot5^2$                 & 0,151        & 0,516        & Não    \\
			3600   & $2^4\cdot3^2\cdot5^2$               & 0,130        & 0,992        & Não    \\
			7200   & $2^5\cdot3^2\cdot5^2$               & 0,800        & 0,563        & Não    \\
			12000  & $2^5\cdot3\cdot5^3$                 & 0,254        & 0,330        & Não    \\
			15000  & $2^3\cdot3\cdot5^4$                 & $1{,}97\times10^{-5}$  & 0,160        & KS sim \\
			18000  & $2^4\cdot3^2\cdot5^3$               & $4{,}37\times10^{-3}$  & 0,054        & KS sim \\
			21000  & $2^3\cdot3\cdot5^3\cdot7$           & $6{,}79\times10^{-23}$ & $3{,}29\times10^{-26}$ & Sim    \\
			24000  & $2^6\cdot3\cdot5^3$                 & $4{,}49\times10^{-5}$  & 0,078        & KS sim \\
			27000  & $2^3\cdot3^3\cdot5^3$               & $1{,}77\times10^{-2}$  & 0,498        & KS sim \\
			30030  & $2\cdot3\cdot5\cdot7\cdot11\cdot13$ & $5{,}03\times10^{-151}$& $2{,}04\times10^{-182}$& Sim    \\
			83160  & $2^3\cdot3^3\cdot5\cdot7\cdot11$   & $2{,}75\times10^{-188}$& $9{,}49\times10^{-205}$& Sim    \\
			100000 & $2^5\cdot5^5$                       & $4{,}04\times10^{-41}$ & $2{,}36\times10^{-54}$ & Sim    \\
			500000 & $2^5\cdot5^6$                       & $1{,}04\times10^{-65}$ & $1{,}49\times10^{-98}$ & Sim    \\
			\bottomrule
		\end{tabular}
		\caption{Testes KS e Mann-Whitney para $\Omega_N(P_1)$ vs.\ $\Omega_N(P_2)$ em $G_{3,N}$.}
		\label{tab:omega}
	\end{table}
	
	\subsection{Interpretação}
	
	\paragraph{Direção do efeito.}
	Em todos os $N$ testados, a média de $\Omega_N$ segue o padrão
	\[
	\overline{\Omega}_N(\mathbb{P}_1)
	\;<\; \overline{\Omega}_N(\mathbb{P}_2)
	\;<\; \overline{\Omega}_N(\text{composto}).
	\]
	Isso é contraintuitivo à primeira vista: poder-se-ia esperar que primos
	``atraíssem'' vizinhos primos na grade. A explicação é que, à medida que
	$N$ cresce, os primos tornam-se mais raros (densidade $\sim 2/\!\ln n$),
	de modo que os \emph{vizinhos} de um primo têm probabilidade decrescente
	de também serem primos — menor, em média, do que a probabilidade dos
	vizinhos de um $\mathbb{P}_2$, que pode residir numa região com padrão
	modular mais favorável. Para a viabilidade do crivo geométrico, o que importa
	não é a direção do efeito, mas o fato de que as distribuições de $\Omega_N$
	sobre $\mathbb{P}_1$ e $\mathbb{P}_2$ são \emph{estatisticamente distinguíveis}
	— e é exatamente isso que os testes confirmam para $N$ suficientemente grande.
	
	\paragraph{Dependência em $N$ e efeitos modulares.}
	Para $N$ moderados com fatorações envolvendo apenas os primos $2$, $3$ e $5$
	(e.g.\ $N \in \{1800, 2400, 3600, 7200, 12000\}$), nenhum teste rejeitou a
	igualdade das distribuições. A interpretação natural é que tais $N$ induzem
	padrões de congruência na grade que homogeneízam $\Omega_N$ entre as classes,
	obscurecendo a diferença. Esse fenômeno é conceitualmente alinhado com o
	\emph{W-trick} da Seção~10.2: a presença exclusiva de pequenos fatores primos
	em $N$ cria obstruções modulares que o filtro $W$ foi projetado para remover.
	
	Para $N = 15\,000$ e $N = 18\,000$ (também da forma $2^a3^b5^c$), o KS já
	detecta diferença na forma global das distribuições, mas o MW não detecta
	diferença de posição ($p = 0{,}160$ e $p = 0{,}054$, respectivamente) —
	comportamento de borda que indica separação parcial captada pelo teste mais
	sensível a caudas, mas não pelo teste de rank. Trata-se de efeito de escala
	finita que desaparece para $N$ maiores.
	
	Quando a fatoração de $N$ inclui um primo $\geq 7$ (e.g.\ $N = 21\,000 =
	2^3\cdot3\cdot5^3\cdot7$ ou $N = 30\,030 = 2\cdot3\cdot5\cdot7\cdot11\cdot13$)
	ou quando $N \gtrsim 10^5$, ambos os testes rejeitam a igualdade com
	$p$-valores extremamente baixos. O caso $N = 30\,030$ — o primorial
	$p_6\# / p_0\#$ — é particularmente expressivo: a presença de todos os
	primos até 13 na fatoração elimina as obstruções modulares e produz a maior
	separação observada na amostra.
	
	\paragraph{Implicação para o crivo geométrico.}
	Esses resultados constituem \textbf{evidência computacional preliminar} de que
	$\Omega_N(n)$ carrega informação estatisticamente relevante sobre a classe
	aritmética de $n$ — primos vs.\ semiprimos — quando $N$ é livre de obstruções
	modulares triviais. Isso é necessário (mas não suficiente) para que o estimador
	$\hat{\pi}_2(a, N)$ da equação~(7) possa, em princípio, distinguir $A_p$ de
	$A_p \cup B_p$ na linguagem da Seção~9.
	
	\begin{remark}[Limitações da evidência computacional]
		Os dados são preliminares e devem ser lidos com cautela:
		\begin{enumerate}
			\item A significância estatística não implica que $\Omega_N$ seja
			\emph{computável sem oráculo de primalidade} — a circularidade do Nó
			Computacional (Seção~11.4) permanece intacta.
			\item Os testes comparam distribuições marginais de $\Omega_N$; não testam
			diretamente se o estimador $\hat{\pi}_2(a,N)$ separa partições de Goldbach
			de não-partições.
			\item O comportamento para $N$ com obstruções modulares reforça a necessidade
			do W-trick antes de qualquer aplicação do crivo geométrico.
		\end{enumerate}
		Esses resultados são apresentados como motivação empírica para a formalização
		ergódica da Seção~10 e o programa do crivo geométrico da Seção~11, não como
		validação de uma prova.
	\end{remark}
	...
	\begin{enumerate}
		...
		\item A \textbf{an\'alise de crivos} (Se\c{c}\~ao~9) revela que o fator $\ln N$
		...
		\item A \textbf{formaliza\c{c}\~ao erg\'odica} (Se\c{c}\~ao~10) traduz a exist\^encia
		de configura\c{c}\~oes primas em problema de recorr\^encia m\'ultipla no sistema
		$(X, \mu_N, T)$, formula as Hip\'oteses~\ref{hyp:ergodica}--\ref{hyp:cobertura-ergodica}
		em linguagem rigorosa e identifica a lacuna precisa entre ``densidade nula do conjunto
		excepcional'' (o que a ergodicidade pode provar) e ``conjunto excepcional vazio''
		(o que Goldbach exige).
		
		\item A \textbf{verifica\c{c}\~ao computacional} (Se\c{c}\~ao~12) testa o peso
		geom\'etrico $\Omega_N(n)$ em grades $G_{3,N}$ para $N$ at\'e $5\times 10^5$,
		mostrando que as distribui\c{c}\~oes de $\Omega_N$ sobre $\mathbb{P}_1$ e
		$\mathbb{P}_2$ s\~ao estatisticamente distingu\'iveis (testes KS e Mann--Whitney)
		para $N$ livre de obstru\c{c}\~oes modulares triviais --- evid\^encia emp\'irica
		de consist\^encia com a hip\'otese do crivo geom\'etrico.
	\end{enumerate}
	
	A lacuna central permanece, agora enunciada em \emph{cinco} linguagens equivalentes:
	...
	(iv) passar de ``conjunto excepcional de densidade nula'' para ``conjunto vazio''
	(teoria erg\'odica);
	(v) tornar $\Omega_N$ comput\'avel sem or\'aculo de primalidade e provar
	$\hat{\pi}_2(a,N)\geq 1$ para todo $a$ (crivo geom\'etrico).
	
	A contribui\c{c}\~ao estrutural deste trabalho \'e mostrar que estas formula\c{c}\~oes
	s\~ao facetas do mesmo objeto, e que a geometria da grade oferece um mecanismo
	natural --- pesos n\~ao-multiplicativos e parity-free ---
	...
	\begin{thebibliography}{9}
		...
		\bibitem{bandeira2026propriedade}
		T. Bandeira,
		``Uma propriedade de soma constante na grade $3\times N$ e as conjecturas
		de Goldbach,''
		\emph{Preprint}, maio de 2026.
		
		\bibitem{bandeira2026ancora}
		T. Bandeira,
		``O Invariante \^Ancora da Grade $G_{3,N}$ e um Estimador Geom\'etrico
