...
\begin{document}
	...
	\begin{abstract}
		...
		\begin{itemize}
			...
			em $W_{i^*}$ e a sua cobertura completa pela permutação
			...
			extremos de $W_{i^*}$ são primos, ou na forma
	...
	\begin{proposition}[Órbita completa de $\sigma$]
		...
		na fita.  Para $t=1,\ldots,N-1$, independentemente de primalidade:
		\begin{enumerate}[(i)]
			...
			\[
			t \;\mapsto\;
			\text{par }(e_1(t),e_3(t))\text{ de }W_{i^*}
			\text{ sob }\sigma^t
			\]
	...
	\begin{proof}
		...
		Após $t$ passos, esses são substituídos pelos das posições
		$i^*+t,\,i^*+C+t,\,i^*+2C+t$.  À medida que $t$ percorre
		$\{1,\ldots,N-1\}$, essas triplas percorrem exactamente os $N-1$
		pares diagonais interiores de $\mathcal{F}_N$, sem repetição.
		
		\textbf{(iii)}  O par $(e_1(t),e_3(t))$ corresponde, via $\Phi$,
		à coluna $k^*+t$ da fita.  Pela Proposição~\ref{prop:soma} com
		$\varepsilon=1$: $e_1(t)+e_3(t)=4N-2=2M^-$.
	\end{proof}
	...
	\begin{remark}[Estrutura direccional de $\sigma$ por linha e natureza dos eventos]
		\label{rem:sigma-direcoes}
		A permutação $\sigma$ não é apenas uma bijecção abstracta: ela
		impõe uma \emph{estrutura direccional distinta em cada linha}
		da grade, visível na evolução dos extremos de $W_{i^*}$.
		
		Fixada $W_{i^*}$, os três elementos $(e_1(t),\,p_2(t),\,e_3(t))$
		evoluem passo a passo segundo os sentidos do percurso zigzag:
		\begin{itemize}
			...
			do centro de~L2 (L2 percorre-se da direita para a esquerda,
			pelo que o elemento de $W_{i^*}$ em L2 \emph{acompanha}~L3
			no sentido geral);
			\item \textbf{L3} ($\rightarrow$, mas \emph{em sentido contrário}
			ao efeito sobre $W_{i^*}$):
			$e_3(t{+}1)=e_3(t)-2$
			--- porque $\Phi$ emparelha~L1 com~L3 em sentidos opostos,
			o elemento de $W_{i^*}$ em~L3 decresce enquanto o de~L1
			cresce.
		\end{itemize}
		...
		=e_1(t)+e_3(t),
		\]
		confirmando directamente a constância da soma da
		...
		A simetria $e_1\!\uparrow\,/\,e_3\!\downarrow$ é exactamente o
		mecanismo pelo qual $W_{i^*}$ percorre as \textbf{diagonais
			secundárias} da fita~---~aquelas que cortam de canto
		superior-direito para canto superior-esquerdo (Figura~1).  Um
		deslocamento arbitrário que não respeitasse as direcções de~L1
		e~L3 quebraria esta simetria: os extremos ficariam separados por
		distância $>2$ na fita, e a soma seria $>2M^-$,
		dessincronizando o motor no passo seguinte.
		
		\medskip
		\noindent\textbf{Nota sobre a natureza dos eventos.}
		Os passos $t=1,\ldots,N-1$ da órbita de $\sigma$ são
		\emph{eventos}~---~todas as configurações possíveis de $W_{i^*}$
	...
	\begin{proposition}[Pares diagonais da fita e unicidade do passo mínimo]
		...
		\textbf{pares diagonais interiores} de $\mathcal{F}_N$ como os
		pares
		\[
		(a_k,\;b_{k+1}^{(0)}),\qquad k=1,\ldots,N-1.
		\]
		...
		da fita é \emph{apontado} por $W_{i^*}$ em exactamente um passo
		...
		\begin{enumerate}[(i)]
			...
			$2M^-+2\delta>2M^-+2=2M^+$, quebrando a cadeia
	...
	\begin{proof}
		...
		$\lceil(N-1)/\delta\rceil$ sobre $W_{i^*}$, estritamente menor
		...
		$e_1$ avança $2\delta$ e $e_3$ recua $2\delta$, logo
		$(e_1(t{+}\delta),e_3(t{+}\delta))$ corresponde à
		coluna $k^*+\delta$ da fita.  O par prometido teria soma
		$4N-2+2\delta$ em vez de $4N-2+2=4N$, implicando um passo
		do motor de $2\delta$ em vez de~$2$.\qed
	\end{proof}
	...
	Para $C=10$, $N=15$, $2M^-=58$.
	A janela central é $W_5$ ($i^*=5$); os elementos iniciais em
	$W_{i^*}$ estão nas posições $5,\,15,\,25$ do percurso zigzag:
	$\sigma_G(5)=9$, $\sigma_G(15)=31$, $\sigma_G(25)=49$.
	
	A tabela abaixo mostra os primeiros passos da órbita de $\sigma$
	sobre os pares $(e_1(t),\,e_3(t))$ de $W_{i^*}$.
	...
	\begin{array}{c|cc|c|c}
		...
		\hline
		1 & 11 & 47 & 58 & \text{sim}\;\checkmark\\
		2 & 13 & 45 & 58 & \text{não}\\
		3 & 15 & 43 & 58 & \text{não (15 composto)}\\
		4 & 17 & 41 & 58 & \text{sim}\;\checkmark\\
		5 & 19 & 39 & 58 & \text{não}\\
		\vdots & \vdots & \vdots & \vdots & \vdots
	\end{array}
	\]
	
	Os extremos $1$ e $6C-1=59$ estão excluídos de $S$ e não entram em
	$W_{i^*}$ ao longo da órbita~---~confirmando que o ciclo fecha
	correctamente após $N-1=14$ passos.  A soma $e_1(t)+e_3(t)=58$ é
	constante para todo~$t$ (Proposição~\ref{prop:orbita}(iii)).
	...
	\begin{definition}[Hipóteses Restritivas]
		...
		$(e_1(t),\,e_3(t))$ os extremos de $W_{i^*}$ sob $\sigma^t$.
		...
		e_3(t)\in\mathbb{P}.
		\]
		Em alguma iteração, \emph{ambos} os extremos de $W_{i^*}$ são
		primos.  Esta é a condição mínima para o motor: o par
		$(e_1(t),e_3(t))$ é então um par prometido (Definição~\ref{def:prometido})
		que, após o deslocamento mínimo, torna-se par base para
		$2M^-+2=2M^+$.
		...
		\]
		Existe uma iteração em que a \emph{tripla completa} de $W_{i^*}$
		(diagonal $D_1$, $D_2$ ou coluna central $C_v$) é inteiramente
		formada por primos.  HR$^+$ implica HR$^-$.  Esta condição mais
		forte conecta ao peso oracle-free $\Omega_N^*$~\cite{peso} e à
	...
	\begin{theorem}[Motor de Herança Estrutural]
		...
		\begin{itemize}
			...
			$(e_1(t),e_3(t))$ de $W_{i^*}$ são ambos primos.
		\end{itemize}
	...
	\begin{itemize}
		...
		Janela central $W_4$ ($i^*=4$, $C=8$):
		configuração $\mathcal{E}(t)$ para $t=1$ dá
		$(e_1,e_3)=(7,41)$, ambos primos
		($7+41=48=2M^+$~\checkmark)~---~evento restritivo HR$^-$.
		Configuração $t=2$ dá $(11,37)$: $11+37=48$~\checkmark,
		também evento restritivo.
	...
	\begin{enumerate}
		...
		\item A \textbf{órbita completa} de $\sigma$: os $N-1$ pares
		diagonais são cobertos em bijecção, com soma constante
		$2M^-$ em todo passo (Proposição~\ref{prop:orbita}).
