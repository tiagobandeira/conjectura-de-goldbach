...
\begin{document}
	...
	\begin{abstract}
		...
		A Hipótese Restritiva Fraca ($\HRm$) — existência de um par de primos na
		órbita da permutação deslizante $\sigma$ — é traduzida, de forma
		incondicional, para a condição $\widehat{F}(0) > 0$: positividade da
		componente de frequência zero da função indicadora de colisão
		$F(t) = \mathbf{1}_{\PP}(e_1(t))\cdot\mathbf{1}_{\PP}(e_3(t))$ no
	...
	A janela central $W_{i^*}$ (com $i^* = \lfloor C/2 \rfloor$) percorre,
	sob a órbita de $\sigma$, exatamente os $N-1$ pares diagonais
	$(e_1(t), e_3(t))$ com $e_1(t) + e_3(t) = 2M^- = 4N - 2$ — resultado
	incondicional \cite[Prop.~4.3, Prop.~4.6]{motor}.
	
	\begin{remark}[Equações de movimento]
		\label{rem:movimento}
		Por construção do percurso zigzag, os extremos de $W_{i^*}$ evoluem
		linearmente:
		\[
		e_1(t) = e_1(0) + 2t,\qquad e_3(t) = e_3(0) - 2t,
		\]
		de modo que $e_1(t) + e_3(t) = e_1(0) + e_3(0) = 2M^-$ é constante
		para todo $t \in \{1,\ldots,N-1\}$.
	\end{remark}
	...
	\begin{definition}[Hipóteses Restritivas]
		\label{def:HR}
		Fixados $C$ e $i^* = \lfloor C/2 \rfloor$, sejam $(e_1(t), e_3(t))$ os
		extremos de $W_{i^*}$ sob $\sigma^t$.
		
		\begin{itemize}
			\item \textbf{$\HRm$ (Fraca):} $\exists\, t \in \{1,\ldots,N-1\}$
			tal que $e_1(t) \in \PP$ e $e_3(t) \in \PP$.
			\item \textbf{$\HRp$ (Forte):} $\exists\, t \in \{1,\ldots,N-1\}$
			tal que a tripla $(e_1(t), p_2(t), e_3(t))$ de $W_{i^*}$ é
	...
	\begin{definition}[Espaço de estados $X_N$]
		...
		\]
		Cada estado $t \in X_N$ corresponde biunivocamente ao par $(e_1(t),
		e_3(t))$ via as equações de movimento da Observação~\ref{rem:movimento}.
	\end{definition}
	...
	\begin{definition}[Espaço de Hilbert e produto interno]
		...
		\[
		\ip{\psi}{\phi} = \frac{1}{N-1}\sum_{t=1}^{N-1} \psi(t)\,\overline{\phi(t)}.
		\]
	\end{definition}
	...
	\begin{definition}[Função de colisão $F$]
		...
		na órbita do motor é definida por
		\[
		F(t) := \mathbf{1}_{\PP}(e_1(t))\cdot\mathbf{1}_{\PP}(e_3(t)),
		\]
	...
	\begin{definition}[Função de peso de von Mangoldt]
		...
		\[
		w(t) := \Lambda(e_1(t))\cdot\Lambda(e_3(t)),
		\]
	...
	\begin{proof}
		A equivalência (a)$\Leftrightarrow$(b) é imediata pois $F(t) \in
	...
	\begin{proposition}[Descorrelação espectral — incondicional]
		...
		a função de peso $w(t) = \Lambda(e_1(t))\,\Lambda(e_3(t))$ satisfaz
		\[
	...
	\begin{proof}
		...
		\[
		L_1(t) = e_1(0) + 2t,\qquad L_2(t) = e_3(0) - 2t,
		\]
		...
		$L_1(t) + L_2(t) = 2M^-$ constante.
		
		Pelo teorema principal de Green-Tao-Ziegler
		\cite{greentaoziegler} sobre a contagem de soluções de sistemas
		lineares em primos (equivalentemente, sobre a ortogonalidade do
		produto $\prod_i \Lambda(L_i(t))$ a nilsequências de grau finito),
		para qualquer nilsequência $\psi$ de grau finito,
		\[
		\frac{1}{K}\sum_{t=1}^{K} \Lambda(L_1(t))\,\Lambda(L_2(t))\,\psi(t) \to 0
		\quad (K \to \infty).
		\]
		Para cada $j \neq 0$, a autofunção $\phi_j(t) = e^{2\pi i j t/K}$ é
		uma nilsequência de grau $1$ (caractere linear sobre $\Z_K$).
		Tomando $\psi(t) = \overline{\phi_j(t)}$, obtemos
		\[
		\widehat{w}(j)
		= \frac{1}{K}\sum_{t=1}^{K} \Lambda(e_1(t))\,\Lambda(e_3(t))\,
		\overline{\phi_j(t)} \to 0. \qedhere
		\]
	\end{proof}
	...
	\begin{remark}[A barreira remanescente — formulação precisa]
		...
		\frac{1}{K}\sum_{t=1}^K \Lambda(e_1(t))\,\Lambda(e_3(t)) \gg
		\frac{1}{(\log K)^2}.
		\]
		Esta estimativa é, do ponto de vista técnico,
		\emph{equivalente à própria Conjectura de Goldbach} na linguagem da
		análise harmônica. A reformulação não simplifica essa dificuldade;
		ela a expressa num objeto — a componente de frequência zero de $w$
		no sistema de Koopman — para o qual as técnicas de nilsequências e
		médias de Weyl têm tração natural.
	\end{remark}
