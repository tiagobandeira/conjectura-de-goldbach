...
\title{Leis de Escala e a Conjectura da Âncora Absoluta:\\ Validação Empírica do Motor de Herança Estrutural até $10^9$}
...
\begin{document}
	...
	\begin{abstract}
		Este artigo consolida os resultados experimentais obtidos com o Motor de Herança Estrutural operando no regime absoluto (multieixo). Apresentamos evidências numéricas até $N = 10^9$ (i.e., $2M \le 6\cdot10^9$) que levam à formulação da \textbf{Conjectura da Âncora Absoluta}: todo número par suficientemente grande pode ser escrito como soma de dois primos onde o menor primo é limitado por $O(\log M \log\log M)$. São derivadas leis de escala empíricas para o número de âncoras necessárias ($k^* \sim 6,3\log N$) e para o maior primo utilizado ($p_{\max} \approx 11,8 \log N \log\log N$). Conectamos esses achados à reformulação espectral do Motor (Artigo 6), mostrando que a evidência empírica é compatível com a positividade da componente de frequência zero do operador de Koopman, enquanto as componentes não nulas são descorrelacionadas. O artigo fornece uma base quantitativa para a Hipótese Restritiva Fraca $HR^-$ e delineia um roteiro para futuras investigações analíticas.
	\end{abstract}
	...
	\begin{quote}
		...
		\begin{enumerate}
			...
			\item O primo $p$ pode ser encontrado pelo \textbf{processo guloso} (testar os primos em ordem crescente) e o número total de primos necessários para cobrir \emph{todos} os pares até $2M$ é assintoticamente $k^*(M) \sim \kappa \cdot \log M$, com $\kappa \approx 6,3$.
		\end{enumerate}
		Os dados numéricos até $N=10^9$ sugerem que podemos tomar $C_0 = 10^7$ e $A = 12$. A constante $A$ efetiva estabiliza em torno de $11$--$12$ para $N \ge 10^8$, com sinais claros de convergência entre $N=5\times10^8$ e $N=10^9$ ($A=11,81$ e $A=11,83$ respectivamente).
	\end{quote}
	...
	\begin{table}[h]
		...
		\begin{tabular}{cccccc}
			...
			\midrule
			$10^6$         &  269 &  7,42 &  55 & 3,62 \\
			$2\times10^6$  &  269 &  6,93 &  55 & 3,79 \\
			$5\times10^6$  &  359 &  8,51 &  70 & 4,54 \\
			$10^7$         &  449 & 10,02 &  85 & 5,27 \\
			$2\times10^7$  &  449 &  9,46 &  85 & 5,06 \\
			$5\times10^7$  &  509 &  9,99 &  95 & 5,36 \\
			$10^8$         &  587 & 10,94 & 105 & 5,70 \\
			$2\times10^8$  &  643 & 11,40 & 115 & 6,02 \\
			$5\times10^8$  &  709 & 11,81 & 125 & 6,24 \\
			$10^9$         &  743 & 11,83 & 130 & 6,27 \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\[
	k^*(N) \approx 6,3 \log N + \text{constante}, \qquad R^2 > 0,95.
	\]
	Este crescimento logarítmico é extremamente favorável: para $N=10^9$ observou-se $k^* = 130$ âncoras, consistente com a lei.
	
	\subsection{Maior âncora $p_{\max}(N)$}
	
	O lado direito da Figura~\ref{fig:leis} apresenta o coeficiente $A_{\text{abs}}(N) = p_{\max}(N)/(\log N\log\log N)$. Após uma deriva inicial, o coeficiente estabiliza na faixa $11$--$12$ para $N\ge 10^8$. O valor em $N=5\times10^8$ é $11{,}81$ e em $N=10^9$ é $11{,}83$ — variação de apenas $0{,}2\%$ entre essas duas últimas escalas, forte indicativo de convergência. A extrapolação sugere:
	\[
	p_{\max}(N) \sim (11{,}8 \pm 0{,}1) \cdot \log N \cdot \log\log N.
	\]
	...
	\begin{enumerate}
		\item \textbf{Extensão computacional} até $N=10^{10}$ para verificar a convergência de $A_{\text{abs}}$ acima de $12$. Os dados atuais até $N=10^9$ mostram $A=11{,}83$, com variação de apenas $0{,}2\%$ entre $N=5\times10^8$ e $N=10^9$.
	...
	O Motor de Herança Estrutural, operando sob a Regra Absoluta, exibe leis de escala claras e robustas: o número de âncoras necessárias para cobrir todos os números pares até $N$ cresce como $6{,}3\log N$, e o maior primo utilizado cresce como $11{,}8 \log N \log\log N$. Esses resultados, obtidos por simulação exaustiva até $10^9$, sustentam a \textbf{Conjectura da Âncora Absoluta}: a decomposição de Goldbach pode sempre ser feita com um primo logaritmicamente pequeno. A convergência de $A_{\text{abs}}$ entre $N=5\times10^8$ e $N=10^9$ ($11{,}81$ e $11{,}83$ respectivamente) é evidência forte de estabilização assintótica.
